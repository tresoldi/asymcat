...
Original file is located at
    https://colab.research.google.com/github/tresoldi/asymcat/blob/master/docs/Demo.ipynb
...
def plot_scorer(scorer, alpha_x, alpha_y, title=None, font_scale=2, figsize=(25, 25)):
    ...
    ax = plt.subplot(111)
    sns.heatmap(matrix, annot=True, fmt='.2f', linewidths=0.5, center=0, ax=ax).set_title(
        title, fontsize=font_scale * figsize[0]
    )
...
mushroom_data = asymcat.read_sequences("docs/mushroom-small.tsv")
...
tresoldi_i = scorer.tresoldi()
tresoldi_ii = scorer.tresoldi()
...
for pair in sorted(scorer.obs):
    buf = [
        ...
        "%0.4f" % chi2_ns[pair][0],
        "%0.4f" % chi2_ns[pair][1],
        "%0.4f" % cramersv[pair][0],
        "%0.4f" % cramersv[pair][1],
        "%0.4f" % cramersv_ns[pair][0],
        "%0.4f" % cramersv_ns[pair][1],
        "%0.4f" % fisher[pair][0],
        "%0.4f" % fisher[pair][1],
        "%0.4f" % theil_u[pair][0],
        "%0.4f" % theil_u[pair][1],
        "%0.4f" % tresoldi_i[pair][0],
        "%0.4f" % tresoldi_i[pair][1],
        "%0.4f" % tresoldi_ii[pair][0],
        "%0.4f" % tresoldi_ii[pair][1],
    ]
    table.append(buf)
...
xy, yx, alpha_x, alpha_y = asymcat.scorer.scorer2matrices(tresoldi_ii)
